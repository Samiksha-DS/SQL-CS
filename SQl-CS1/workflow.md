...
-- 2. Download "DataCo Smart Supply Chain for Big Data Analysis" dataset
-- 3. Read raw CSV into a DataFrame using Latin-1 encoding
...
--    (safely handles special characters like # and @ in password)
-- 3. Connect to MySQL server without specifying a database first
-- 4. Run: CREATE DATABASE IF NOT EXISTS supply_chain_db
-- 5. Reconnect, this time targeting supply_chain_db directly
-- 6. Read staged_shipments.csv into a DataFrame
-- 7. Write DataFrame into raw_shipments table
--    (replace if exists, load in chunks)
...
--       is_late     = 1 IF actual_shipping_days > scheduled_shipping_days
--                     ELSE 0
--       carry forward: order_id, order_date, shipping_mode,
--                       region, country, category_name, order_sales
...
--       Compute : total_shipments, late_shipments,
--                 late_rate_pct, on_time_rate_pct,
--                 avg_delay_days, STDDEV(delay_days) AS delay_variability
--
-- 2. Region x shipping-mode risk concentration
--       GROUP BY  order_region, shipping_mode
--       HAVING    COUNT(*) >= 50   -- filter out noisy small samples
--       Compute : late_rate_pct, avg_delay_days
--       Sort    : descending by late_rate_pct, limit top 15
--
...
--       Sort    : descending by revenue_at_risk, limit top 10
--
-- 4. Composite scorecard (final deliverable)
--       Step 1 : build mode_stats CTE
--                 -> on_time_pct, avg_delay_days, delay_stddev
--                    per shipping_mode
--       Step 2 : apply RANK() OVER (
...
-- 2. Cross-check any striking result (e.g., 0% on-time, 0 variance)
--    against a regional breakdown before treating it as a real finding
-- 3. Write quantified, ranked takeaways into insights.md
